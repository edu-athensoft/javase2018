 ...
 * 
 * The essence of generics is to parameterize types (the types of formal
 * constraints that are controlled by different types of generics without
 * creating new types). That is to say in the process of generic use, the data
 * type of the operation is specified as a parameter, this type of parameters
 * can be used in classes, interfaces and methods, respectively known as generic
 * classes, generic interfaces, generic methods
 */
...
public class GenericsType {
	...
	public void introduceGeneric() {
		...
		/**
		 * now there is a warning in Box2 Class says : Box2 is a raw type.
		 * References to generic type Box2<T> should be parameterized.
		 */
		Box2 box2 = new Box2();
		...
		box2Integer.setT(1);
		// box2Integer.setT("1");
		// Error : The method setT(Integer) in the type Box2<Integer> is not
		// applicable for the arguments (String)

		/**
		 * once we set a type as a parameter of <T> , the compiler only need to
		 * help us to check if there are compilation problem ,otherwise a lot of
		 * class casting and auto-boxing work will have to be done.
		 * 
		 */
 ...
 * 
 * Begin by examining a non-generic Box class that operates on objects of any
 * type.
 * 
 */
class Box {
